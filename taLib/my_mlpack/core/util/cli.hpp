 ...
 *     new paragraphs).
 */
#define PROGRAM_INFO(NAME, DESC) static ta::util::ProgramDoc \
    io_programdoc_dummy_object = ta::util::ProgramDoc(NAME, DESC);
 ...
 */
#ifdef __COUNTER__
  #define PARAM(T, ID, DESC, ALIAS, DEF, REQ) static ta::util::Option<T> \
  ...
  #define PARAM_FLAG_INTERNAL(ID, DESC, ALIAS) static \
      ta::util::Option<bool> JOIN(__io_option_flag_object_, __COUNTER__) \
      (ID, DESC, ALIAS);
  /** @endcond */
  ...
  /** @cond Don't document internal macros. */
  #define PARAM_FLAG_INTERNAL(ID, DESC, ALIAS) static \
      ta::util::Option<bool> JOIN(__io_option_flag_object_, __LINE__) \
      (ID, DESC, ALIAS);
  /** @endcond */
