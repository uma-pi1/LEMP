...
class MyCoverTree
{
 public:
   ...
   */
  MyCoverTree(const arma::mat& dataset, int threads,
            const double base = 2.0,
            VectorMatrix* my_matrix=NULL, size_t startPos=0, size_t endPos=0, 
   ...
   */
  MyCoverTree(const MyCoverTree& other);
  ...
  const MyCoverTree& Child(const size_t index) const { return *children[index]; }
  //! Modify a particular child node.
  MyCoverTree& Child(const size_t index) { return *children[index]; }

  //! Get the number of children.
  size_t NumChildren() const { return children.size(); }

  //! Get the children.
  const std::vector<MyCoverTree*>& Children() const { return children; }
  //! Modify the children manually (maybe not a great idea).
  std::vector<MyCoverTree*>& Children() { return children; }
  ...
  double MinDistance(const MyCoverTree* other) const;

  //! Return the minimum distance to another node given that the point-to-point
  //! distance has already been calculated.
  double MinDistance(const MyCoverTree* other, const double distance) const;
  ...
  double MaxDistance(const MyCoverTree* other) const;

  //! Return the maximum distance to another node given that the point-to-point
  //! distance has already been calculated.
  double MaxDistance(const MyCoverTree* other, const double distance) const;
  ...
  math::Range RangeDistance(const MyCoverTree* other) const;
  ...
  math::Range RangeDistance(const MyCoverTree* other, const double distance)
      const;
  ...
  //! Get the parent node.
  MyCoverTree* Parent() const { return parent; }
  //! Modify the parent node.
  MyCoverTree*& Parent() { return parent; }
  ...
  VectorMatrix* my_matrix; // this is to hack in my implementation
  size_t startPos, endPos; // tree will be built for only some part of the matrix
 ...
 private:
  ...
  std::vector<MyCoverTree*> children;
  ...
  StatisticType stat; // vector to manage multiple threads
