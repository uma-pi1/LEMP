...
class MyCoverTree<MetricType, RootPointPolicy, StatisticType>::SingleTreeTraverser
{
 public:
  /**
   * Initialize the single tree traverser with the given rule.
   */
  SingleTreeTraverser(std::vector<RuleType >& rule);
   ...
   */
  void Traverse(const size_t queryIndex, MyCoverTree& referenceNode, std::vector<QueueElement>& results);
  

  void getStats(const size_t queryIndex,std::vector<QueueElement>& results,
                MyCoverTree& referenceNode,
                double& rootChildScore, double& rootBaseCase, int& firstLevelToSearch);
          
          
  void TraverseForTheta(const size_t queryIndex, MyCoverTree& referenceNode,
