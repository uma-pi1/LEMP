...
int main() {
    ...
    InputArguments args;
//    args.theta = 0.035;//0.035; // theta for the Above-theta problem (will be used when running .runAboveTheta())
    ...
    LEMP_Method method = LEMP_LI; // choose one of the methods LEMP provides (LEMP_X where X: L, LI, LC, I, C, TA, TREE, AP, LSH)
    int cacheSizeinKB = 2048; // specify your cache size per core (important since LEMP tries to optimize for cache utilization)
    double R = 0.60; // recall parameter for LEMP_LSH
    bool isTARR = true; // relevant for LEMP_TA (true will use ROUND ROBIN, false will use a MAX HEAP)
    double epsilon = 0.3; // epsilon value for LEMP_LI with absolute or relative approximation
...
//        algo.setMethod(LEMP_LSH); // we might also change the method if we want so
//        results.clear();
//        
