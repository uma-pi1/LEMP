...
#include <mips/my_mlpack/core/util/string_util.hpp>
#include <string>
...
namespace mips {
    namespace tree {
        ...
        MyCoverTree<MetricType, RootPointPolicy, StatisticType>::MyCoverTree(
                const arma::mat& dataset, int threads,
                const double base,
                VectorMatrix* my_matrix, size_t startPos, size_t endPos, 
                MetricType* metric) :
        dataset(dataset),
        ...
        metric(metric),
        distanceComps(0),
        my_matrix(my_matrix), startPos(startPos), endPos(endPos), threads(threads) {
            ...
            //		  std::cout<<"distances: "<<distances.n_rows<<std::endl;
            //
            ...
            // If there is only one point in the dataset... uh, we're done.
            if (my_matrix->rowNum == 1 || endPos - startPos == 1)
                return;

            // Kick off the building.  Create the indices array and the distances array.
            arma::Col<size_t> indices = arma::linspace<arma::Col<size_t> >(startPos + 1,
                    endPos - 1, endPos - 1 - startPos);

            //		  		  std::cout<<indices.n_cols<<" x "<<indices.n_rows<<std::endl;
            //		  		  std::cout<<indices(0)<<" "<<indices(endPos - 2)<<std::endl;

            // This is now [1 2 3 4 ... n].  We must be sure that our point does not
            // occur.
            // I here overwrite the policy. I use the first point is the root but I change it to startPos
            point = startPos;
            //		  if (point != 0)//////
            //		    indices[point - 1] = 0; // Put 0 back into the set; remove what was there.

            arma::vec distances(endPos - 1 - startPos);


            // Build the initial distances.
            MyComputeDistances(point, indices, distances, endPos - 1 - startPos);


            // Create the children.
            size_t farSetSize = 0;
            size_t usedSetSize = 0;
            MyCreateChildren(indices, distances, endPos - 1 - startPos, farSetSize,
                    usedSetSize, threads);
        ...
        my_matrix(my_matrix), startPos(startPos), endPos(endPos), threads(threads) {
            ...
            if (my_matrix == NULL) {
                ...
                CreateChildren(indices, distances, nearSetSize, farSetSize, usedSetSize, threads);
            } else {
                MyCreateChildren(indices, distances, nearSetSize, farSetSize, usedSetSize, threads);
            }
        ...
        metric(other.metric),
        distanceComps(0),
        my_matrix(other.my_matrix), startPos(other.startPos), endPos(other.endPos), threads(other->threads) {

            //stat.resize(threads);
            // Copy each child by hand.
            for (size_t i = 0; i < other.NumChildren(); ++i) {
                children.push_back(new MyCoverTree(other.Child(i)));
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        inline size_t
        MyCoverTree<MetricType, RootPointPolicy, StatisticType>::NumDescendants() const {
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        inline size_t
        MyCoverTree<MetricType, RootPointPolicy, StatisticType>::Descendant(
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        double MyCoverTree<MetricType, RootPointPolicy, StatisticType>::MinDistance(
                const MyCoverTree<MetricType, RootPointPolicy, StatisticType>* other) const {
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        double MyCoverTree<MetricType, RootPointPolicy, StatisticType>::MinDistance(
                const MyCoverTree<MetricType, RootPointPolicy, StatisticType>* other,
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        double MyCoverTree<MetricType, RootPointPolicy, StatisticType>::MaxDistance(
                const MyCoverTree<MetricType, RootPointPolicy, StatisticType>* other) const {
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        math::Range MyCoverTree<MetricType, RootPointPolicy, StatisticType>::
        RangeDistance(const MyCoverTree* other) const {
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        math::Range MyCoverTree<MetricType, RootPointPolicy, StatisticType>::
        RangeDistance(const MyCoverTree* other,
                const double distance) const {
            math::Range result;
            result.Lo() = distance - furthestDescendantDistance -
                    other->FurthestDescendantDistance();
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        inline void
        MyCoverTree<MetricType, RootPointPolicy, StatisticType>::CreateChildren(
                arma::Col<size_t>& indices,
                arma::vec& distances,
                size_t nearSetSize,
                size_t& farSetSize,
                size_t& usedSetSize, int threads) {
            ...
            if (maxDistance == 0) {
                ...
                size_t tempSize = 0;
                children.push_back(new MyCoverTree(dataset, threads, base, point, INT_MIN, this, 0,
                ...
                for (size_t i = 0; i < nearSetSize; ++i) {
                    ...
                    children.push_back(new MyCoverTree(dataset, threads, base, indices[i],
            ...
            children.push_back(new MyCoverTree(dataset, threads, base, point, nextScale, this, 0,
            ...
            while (nearSetSize > 0) {
                ...
                if ((nearSetSize == 1) && (farSetSize == 0)) {
                    ...
                    children.push_back(new MyCoverTree(dataset, threads, base, indices[0], nextScale,
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        inline void
        MyCoverTree<MetricType, RootPointPolicy, StatisticType>::MyCreateChildren(
                arma::Col<size_t>& indices,
                arma::vec& distances,
                size_t nearSetSize,
                size_t& farSetSize,
            ...
            if (maxDistance == 0) {
                ...
                size_t tempSize = 0;
                children.push_back(new MyCoverTree(dataset, threads, base, point, INT_MIN, this, 0,
                        indices, distances, 0, tempSize, usedSetSize, *metric, my_matrix, startPos, endPos)); ///////////
                distanceComps += children.back()->DistanceComps();
                ...
                for (size_t i = 0; i < nearSetSize; ++i) {
                    ...
                    children.push_back(new MyCoverTree(dataset, threads, base, indices[i],
                            INT_MIN, this, distances[i], indices, distances, 0, tempSize,
                            usedSetSize, *metric, my_matrix, startPos, endPos));
                    distanceComps += children.back()->DistanceComps();
                    usedSetSize++;
                }
                ...
                // and we want
                // [ far | all used ].
                SortPointSet(indices, distances, 0, usedSetSize, farSetSize); //////////??????????? seems ok
            ...
            // set and far set for the self child.
            size_t childNearSetSize =
                    SplitNearFar(indices, distances, bound, nearSetSize); /////////////////??????????????? seems ok
            ...
            children.push_back(new MyCoverTree(dataset, threads, base, point, nextScale, this, 0,
                    indices, distances, childNearSetSize, childFarSetSize, childUsedSetSize,
                    *metric, my_matrix, startPos, endPos));
            ...
            // Remove any implicit nodes we may have created.
            RemoveNewImplicitNodes(); ///////////??????????????????? seems ok
            ...
            while (nearSetSize > 0) {
                ...
                // Create the near and far set indices and distance vectors.  We don't fill
                // in the self-point, yet.
                arma::Col<size_t> childIndices(nearSetSize + farSetSize); // looks like simple copy. should be fine
                childIndices.rows(0, (nearSetSize + farSetSize - 2)) = indices.rows(1,
                        nearSetSize + farSetSize - 1);
                ...
                // Build distances for the child.
                MyComputeDistances(indices[0], childIndices, childDistances, nearSetSize
                        + farSetSize - 1);
                ...
                children.push_back(new MyCoverTree(dataset, threads, base, indices[0], nextScale,
                        this, distances[0], childIndices, childDistances, childNearSetSize,
                        childFarSetSize, childUsedSetSize, *metric, my_matrix, startPos, endPos)); ////////////////
                ...
                // set in our own vector.
                MoveToUsedSet(indices, distances, nearSetSize, farSetSize, usedSetSize,
                        childIndices, childFarSetSize, childUsedSetSize); ///////////// seems ok
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        size_t MyCoverTree<MetricType, RootPointPolicy, StatisticType>::SplitNearFar(
                arma::Col<size_t>& indices,
                arma::vec& distances,
                const double bound,
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        void MyCoverTree<MetricType, RootPointPolicy, StatisticType>::ComputeDistances(
                const size_t pointIndex,
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        void MyCoverTree<MetricType, RootPointPolicy, StatisticType>::MyComputeDistances(
                const size_t pointIndex,
                const arma::Col<size_t>& indices,
                arma::vec& distances,
                const size_t pointSetSize) {
            ...
            // modified.
            distanceComps += pointSetSize;
            const double* thisVector = my_matrix->getMatrixRowPtr(pointIndex);

            for (size_t i = 0; i < pointSetSize; ++i) {
                distances[i] = my_matrix->L2Distance(indices[i], thisVector);
            }

        }

        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        size_t MyCoverTree<MetricType, RootPointPolicy, StatisticType>::SortPointSet(
                arma::Col<size_t>& indices,
                arma::vec& distances,
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        void MyCoverTree<MetricType, RootPointPolicy, StatisticType>::MoveToUsedSet(
                arma::Col<size_t>& indices,
                arma::vec& distances,
                size_t& nearSetSize,
                size_t& farSetSize,
        ...
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        size_t MyCoverTree<MetricType, RootPointPolicy, StatisticType>::PruneFarSet(
                arma::Col<size_t>& indices,
                arma::vec& distances,
                const double bound,
         ...
         */
        template<typename MetricType, typename RootPointPolicy, typename StatisticType>
        std::string MyCoverTree<MetricType, RootPointPolicy, StatisticType>::ToString()
        const {
            ...
            if (IsLeaf() == false) {
                for (int i = 0; i < children.size(); i++) {
                    convert << std::endl << mips::util::Indent(children.at(i)->ToString());
                }
            }
            return convert.str();
        }
